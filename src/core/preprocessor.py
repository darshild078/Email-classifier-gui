...
class TextPreprocessor:
    ...
    def __init__(self, remove_stopwords=True, apply_stemming=True, min_word_length=2):
        # Keep important words that affect spam meaning
    ...
    def _clean_message(self, text: str) -> str:
        ...
        # Replace URLs and emails with meaningful placeholders instead of removing
        text = re.sub(r"http\S+|www\.\S+", " hasurl ", text)
        text = re.sub(r"\S+@\S+", " hasemail ", text)
        
        # Handle phone numbers (expanded patterns)
        ...
        # Convert excessive punctuation to semantic meaning
